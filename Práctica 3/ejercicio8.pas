...
begin
    writeln('Ingrese nombre de la distribucion: ');readln(leerInfo.nombre);
    if (leerInfo.nombre<>'zzz') then begin
        writeln('Ingresa anio de lanzamiento: ');readln(leerInfo.anioLanzamiento);
        //leerInfo.anioLanzamiento:=random(53)+1970;
        writeln('Ingresa version del kernel: ');readln(leerInfo.versionKernel);
        //leerInfo.versionKernel:=random()*15;
        writeln('Ingresa cantidad de desarrolladores: ');readln(leerInfo.cantidadDesarrolladores);
        //leerInfo.cantidadDesarrolladores:=random(500)+1;
        writeln('Ingresa descripcion: ');readln(leerInfo.descripcion);
...
begin
    ...
    while(not eof(name)) do begin
        read(name,distro);
        if (distro.cantidadDesarrolladores>0) then writeln('Nombre de la distribucion: ',distro.nombre,'. Anio de lanzamiento: ',distro.anioLanzamiento,'. Version de kernel: ',distro.versionKernel:0:2,'. Cantidad de desarrolladores: ',distro.cantidadDesarrolladores,'. Descripcion: ',distro.descripcion);
    end;
    close(name);
end;
...
begin
    ...
    while (not eof(name)) do begin
        read(name,distro);
        if (distro.nombre=nombre) then existeDistribucion:=true;
    end;
    close(name);
end;
...
begin
    ...
    if (existeDistribucion(name,nombreBaja)) then begin
        ...
        read(name,cabecera);
        read(name,distro);
        while (distro.nombre<>nombreBaja) do begin
            read(name,distro);
        end;
        seek(name,filepos(name)-1);
        write(name,cabecera);
...
begin
    ...
    writeln('Ingrese el nombre de alguna distribucion para verificar si existe');readln(nombreExiste);
    if (existeDistribucion(name,nombreExiste)) then writeln('La distribucion con nombre ',nombreExiste,' existe en el archivo.')
    else writeln('La distribucion con nombre ',nombreExiste,' no existe en el archivo.');
